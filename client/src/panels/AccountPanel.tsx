...
// Share code = base64 of { username, xp, gold, rebirths } (no password)
...
// Shared profiles (other players imported via share code)
...
export function AccountPanel({
  ...
  const [view, setView] = useState<'menu' | 'login' | 'register' | 'leaderboard' | 'share' | 'rebirth'>('menu')
  ...
  // Combined leaderboard: own accounts + imported shared profiles
  const ownEntries = Object.entries(accounts).map(([u, d]) => ({
    username: u, xp: d.xp, gold: d.gold, rebirths: d.rebirths ?? 0, isShared: false,
  }))
  const sharedEntries = Object.entries(shared)
    .filter(([u]) => !accounts[u]) // don't duplicate
  ...
  const handleGenerateCode = () => {
    if (!loggedIn) return
    const code = makeShareCode(loggedIn.username, currentXp, currentGold, currentRebirths)
    setShareCode(code)
  }
  ...
  const handleImportCode = () => {
    ...
    const updated = { ...shared, [parsed.username.toLowerCase()]: { xp: parsed.xp, gold: parsed.gold, rebirths: parsed.rebirths ?? 0 } }
    saveSharedProfiles(updated)
    ...
    <div className="panel account-panel">
      ...
      {view === 'share' && (
        <>
          ...
          <div className="acc-guest">Share this code with friends so they can add you to their leaderboard.</div>
          {loggedIn ? (
            <>
      ...
      {view === 'rebirth' && (
        <>
          ...
          <div className="rebirth-info">
            ...
            <p>Rebirthing resets your XP to 0 but grants a permanent <strong>+20% damage bonus</strong> per rebirth and a special 🔥 badge.</p>
            <p style={{ marginTop: 8 }}>All your items, gold, and name tags are <strong>kept</strong>.</p>
            <p style={{ marginTop: 8, color: '#ffd54f' }}>Current rebirths: 🔥×{currentRebirths}</p>
            <p style={{ marginTop: 4, color: canRebirth ? '#66bb6a' : '#ef5350' }}>
              {canRebirth ? `✓ You have ${currentXp} XP — ready to rebirth!` : `✗ Need ${REBIRTH_XP_REQUIRED} XP (you have ${currentXp})`}
            </p>
          </div>
          {canRebirth && (
